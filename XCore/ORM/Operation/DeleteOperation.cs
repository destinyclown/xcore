    ...
    internal class DeleteOperation {
        ...
        private static int deleteSingle(int id, EntityInfo entityInfo)
        {
            ...
            if (conn.State == System.Data.ConnectionState.Closed)
            {
                conn.Open();
                OrmHelper.initCount++;
                LogManager.GetLogger("Class:System.ORM.Operation.DeleteOperation Method:deleteSingle").Info("数据库连接已开启【" + OrmHelper.initCount + "】");
            }
            IDbCommand cmd = DataFactory.GetCommand(getDeleteSql(id, entityInfo), conn);
            int rowAffected = cmd.ExecuteNonQuery();
            if (!DbContext.shouldTransaction())
            {
                if (conn.State == ConnectionState.Open)
                {
                    ...
                    conn.Dispose();
                    OrmHelper.clostCount++;
                    LogManager.GetLogger("Class:System.ORM.Operation.DeleteOperation Method:deleteSingle").Info("数据库连接已关闭【" + OrmHelper.clostCount + "】");
                }
            }
            return rowAffected;
        }
