...
namespace System {
    ...
    public class ObjectBase<T> : IEntity, IComparable where T : ObjectBase<T> {
        ...
        /// <param name="dir"></param>
        /// <returns></returns>
        public static T findByField(bool isOr, bool isLike, params System.Data.KeyValueData[] kvs)
        {
            try
            {
                StringBuilder where;
                ...
                if (kvs == null)
                {
                    kvs = new System.Data.KeyValueData[] { };
                }
                foreach (System.Data.KeyValueData kv in kvs)
                {
                    if (isOr)
                    {
                        if (isLike)
                        {
                            where.AppendFormat(" or {0} like '%{1}%'", kv.KeyName, kv.KeyValue);
                        }
                        else
                        {
                        }
                    ...
                    else
                    {
                        if (isLike)
                        {
                            where.AppendFormat(" and {0} like '%{1}%'", kv.KeyName, kv.KeyValue);
                        }
                        else
                        {
        ...
        /// </summary>
        /// <returns>返回一个结果对象 Result。如果发生错误，则 Result 中包含错误信息</returns>
        public Result save() { if (this.Id > 0) { return db.update(this); } else { return db.insert(this); } }
        ...
        public void update(params String[] arrPropertyName ) { db.update( this, arrPropertyName ); }
        ...
        where B : ObjectBase<T>
    {
        ...
        /// 对象Guid标识字段
        /// </summary>
        [Column(Length = 50), NotNull("Sorry,对象Guid不能为空！"), Unique("Sorry,对象Guid已经存在！")]
        public string Guid
        {
            get { return _Guid; }
            set { _Guid = value; }
