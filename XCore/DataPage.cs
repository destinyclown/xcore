...
namespace System {
    ...
    public class DataPage<T> : IPageList {
        ...
        /// <returns></returns>
        public static String GetPageBar<T>(DataPage<T> pager, String baseurl)
        {
            string _PageBarHTML = "";
            if (pager.PageCount >= 1)
            {
                _PageBarHTML += "<div class=\"page\">";
                if (pager.Current == 1)
                {
                    _PageBarHTML += "<span><上一页</span>";
                }
                else
                {
                    _PageBarHTML += "<a href=\"" + baseurl + "?page=" + (pager.Current - 1).ToString() + "\"><上一页></a>";
                }
                int i = 1;
                int min = 0;
                int max = 10;
                ...
                for (; min < max; min++)
                {
                    if ((i + min) == pager.Current)
                    {
                        _PageBarHTML += "<em>" + (i + min) + "</em>";
                    }
                    else
                    {
                        if (baseurl.Contains("?"))
                        {
                            _PageBarHTML += "<a href=\"" + baseurl + "&page=" + (i + min) + "\">" + (i + min) + "</a>";
                        }
                        else
                        {
                            _PageBarHTML += "<a href=\"" + baseurl + "?page=" + (i + min) + "\">" + (i + min) + "</a>";
                ...
                {
                    _PageBarHTML += "<span>下一页></span>";
                }
                else
                {
                    _PageBarHTML += "<a href=\"" + baseurl + "?page=" + (pager.Current + 1).ToString() + "\">下一页></a>";
                }
                _PageBarHTML += "</div>";
            }
            return _PageBarHTML;
